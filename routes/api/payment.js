...
router.get('/', async (req, res) => {
    try {
      const payment = await Payment.find().sort({"date":-1}).populate({path : 'employee_id customer_id '});
...
router.get('/:id/customer', async (req, res) => {
    try {
      const payment = await Payment.find({customer_id:req.params.id}).sort({"date":-1}).populate({path : 'employee_id customer_id'});;
      if (!payment) {
        return res.status(400).json({ msg: 'No Payment found !' });
      }
      res.json(payment);
...
router.post('/', async (req, res) => {
    try {
      ...
      let placed_payment = new Payment({customer_id,employee_id,amount,date});

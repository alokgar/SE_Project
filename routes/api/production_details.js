...
router.get('/', async (req, res) => {
    try {
        const production_details = await Production_details.find({}).populate('product_id size_id');
...
router.post('/', async (req, res) => {
    ...
    const {product_name, size_packing_type, prev_quantity, new_quantity} = req.body;
    try {
